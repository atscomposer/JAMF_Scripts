...
cdialog="/Library/iRobot/CocoaDialog.app"	#please specify the appbundle rather than the actual binary
...
msgpatchoodeploywelcome="Welcome to iRobot IT Deploy.
...
promptInstall()
{
	...
	case $promptmode in
		...
		"--selfservice" )
			...
			#
			answer=$(displayDialog "$message" "$msgtitlenewsoft" "$msgnewsoftware" "package" "Install" "Cancel" )
		;;
			...
			then
				...
				then
					...
					then
						...
						makeMessage ""
						makeMessage "$msginstalllater"
						answer=$(displayDialog "$message" "$msgtitlenewsoft" "$msgnewsoftware" "package" "Later" "Install")
				fi
			else
...
getAppList()
(
	applist="$(sudo -u "ashuttleworth" osascript -e "tell application \"System Events\" to return displayed name of every application process whose (background only is false and displayed name contains \"Microsoft\")")"
	echo "$applist"
	formatlist=$( IFS=$'\n'; echo "${applist[@]}" )
)
...
procArray=( $(for proc in $(getAppList); do echo $proc | sed 's/^ //g' | perl -p -e 's/, /\n/g'; done) )
...
mes2="
Ensure you have saved your documents and quit these open Microsoft Office applications:

$(getAppList)"

displayDialog "$mes2" "Close Applications" "Please Close All Microsoft Office Applications" " " "Close Apps" "Cancel"
}
...
if [ "$myResult" == "Cancel" ]; then
	makeProcList 
fi
if [ "$myResult" == "Close Apps" ]; then 
...
(
	...
	while [ "$(getAppList)" != "" ]
	do
		# if we still haven't quit Apps
		secho "apps are still running, prompting user and trying quit loop again.."
		waitForClose
	done
	...
	# 3mv /System/Library/CoreServices/RemoteManagement/AppleVNCServer.bundle/Contents/Support/LockScreen.app/Contents/Resources/Lock.jpg.backup /System/Library/CoreServices/RemoteManagement/AppleVNCServer.bundle/Contents/Support/LockScreen.app/Contents/Resources/Lock.jpg
